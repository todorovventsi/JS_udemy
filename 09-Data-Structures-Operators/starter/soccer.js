...
for (const [goalNum, name] of game.scored.entries()) {
    console.log(`"Goal ${goalNum + 1}: ${name}"`);
}
...
function convert() {
    ...
    input.forEach((word, index) => {
        const wordsArr = word.trim().split('_');
        const wordCamel =
            wordsArr[0].toLowerCase() +
            wordsArr[1]
                .toLowerCase()
                .replace(wordsArr[1][0], wordsArr[1][0].toUpperCase());
        console.log(wordCamel.padEnd(30) + '✅'.repeat(index + 1));
    });
}
...
for (let flight of flights.split('+')) {
    ...
    let deleyed = status.startsWith('Delayed') ? '🔴' : '';

    console.log(
        `${deleyed}${status} from ${from} to ${to} (${time})`.padStart(43, ' ')
    );
}
